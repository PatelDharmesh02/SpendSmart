...
import { handleDeleteTransaction } from "@/redux/thunk";
import {
...
export default function Transactions() {
  ...
  const storeTransactions = useAppSelector(transactionsSelector).transactions;
  const loading = useAppSelector(transactionLoadingSelector);
  const error = useAppSelector(transactionErrorSelector);
  ...
  const fetchTransactions = useCallback(async () => {
    try {
      const response = await AxiosInstance.get(
        `/transactions/by-month/?month=${formattedMonth}`
      );
      if (response.data) {
  ...
  useEffect(() => {
    fetchTransactions();
  }, [fetchTransactions, storeTransactions]);
  ...
  const handleDelete = async () => {
    const selectedRows = Object.keys(rowSelection).map(
      (index) => transactions[parseInt(index)]
    );
    ...
    setIsDeleting(true);
    try {
      // Delete selected transactions one by one
      for (const transaction of selectedRows) {
        await dispatch(handleDeleteTransaction(transaction.id)).unwrap();
      }

      showToast(
        `Successfully deleted ${selectedRows.length} transaction${
      ...
      handleErrorWithoutHook(error as AppError, showToast, {
        prefix: "Transactions",
        fallbackMessage: "Failed to delete transactions",
      });
    } finally {
      setIsDeleting(false);
    }
  ...
  return (
    <Container>
      ...
      {transactions.length > 0 ? (
        ...
        <NoDataContainer>
          <NoDataText>No transactions available for this month</NoDataText>
        </NoDataContainer>
      )}
    </Container>
  );
